...
# ---------- Common utility: supports dict / SectionProxy / ConfigParser ----------
...
def app_cfg(pytestconfig):
    # Allow CONFIGS["RUN_INI"] to be dict / SectionProxy / ConfigParser
    ini_cfg = CONFIGS.get("RUN_INI")

    # First get INI values, then allow CLI options to override
    ...
    # Optional: temporary override for REST/WS base
    ...
    logger.info(
        ...
        cfg["instrument_name"],
        cfg["depth"],
        cfg["book_subscription_type"],
        cfg["book_update_frequency"],
        cfg["rest_base"],
        cfg["ws_market_url"],
    )
    return cfg
